...
def select(sql, headers = None):
    ...
    if dboptions["host"] != "" or dboptions["banco"] != "" or dboptions["uid"] != "":
        ...
        _connection = sqlanydb.connect(host=dboptions["host"], uid=dboptions["uid"], pwd=dboptions["pwd"], eng=dboptions["servidor"], dbn=dboptions["banco"])
        ...
        tools.Log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        tools.Log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        tools.Log("Arquivo de conexão com o banco de dados não configurado!")
        tools.Log("Por favor, altere o arquivo {}\sybase.conf".format(os.path.dirname(os.path.abspath(__file__))))
        tools.Log("    com os dados necessários e tente novamente")
        tools.Log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        tools.Log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        tools.Log("")
        return None
...
def buscarempresa(codigo = "", nome = "", cnpj = "", grupo = ""):
    ...
    if cnpj != "" :
        ...
        _coma = " AND "
    if grupo != "" :
        _grupo = "JOIN bethadba.usconfempresas AS ue ON (ue.i_empresa = e.codi_emp AND ue.modulos <> '' AND ue.tipo = 3 AND ue.i_confusuario = {})".format(grupo)
    ...
    _sql = "SELECT e.codi_emp, COALESCE(e.nome_emp, '') AS nome, COALESCE(CAST(e.dina_emp AS VARCHAR(10)), '') AS datasaida, COALESCE(e.cgce_emp, '') AS cnpj, COALESCE(e.iest_emp, '') AS ie, COALESCE(e.imun_emp, '') AS im, COALESCE(CAST((SELECT FIRST vi.rfed_par FROM bethadba.EFPARAMETRO_VIGENCIA AS vi WHERE vi.codi_emp = e.codi_emp ORDER BY vi.codi_emp, vi.vigencia_par DESC) AS VARCHAR(20)), '') AS regime, COALESCE(e.esta_emp, '') AS uf, COALESCE((SELECT nome_municipio FROM bethadba.gemunicipio WHERE codigo_municipio = e.codigo_municipio), '') AS mun, COALESCE(e.email_emp, '') AS email, CAST(e.dcad_emp AS varchar(10)) AS inicio, e.stat_emp AS situacao, (SELECT list(usu.i_usuario) FROM bethadba.usconfusuario AS usu JOIN bethadba.usconfempresas AS ue ON (ue.tipo = usu.tipo AND ue.i_confusuario = usu.i_confusuario AND ue.modulos <> '') WHERE usu.tipo = 3 AND ue.modulos <> '' AND ue.i_empresa = e.codi_emp) AS grupo, (SELECT list(i_confusuario) FROM bethadba.usconfempresas AS ue WHERE ue.modulos <> '' AND ue.tipo = 3 AND ue.i_empresa = e.codi_emp) AS grupo_cod, COALESCE(e.i_cnae20, '') AS cnae_cod, cnae = COALESCE((SELECT cnae.descricao FROM bethadba.gecnae20 AS cnae WHERE cnae.i_cnae20 = e.i_cnae20), ''), COALESCE(e.obs_geral, '') AS obs FROM bethadba.geempre AS e {} {} ORDER BY 2".format(_grupo, _where)

    _headers = ["codigo", "nome", "datasaida", "cnpj", "ie", "im", "regime", "uf", "mun", "email", "inicio", "situacao", "grupo", "grupo_cod", "cnae_cod", "cnae", "obs"]
...
def buscarempresas():
    ...
    _headers = ["codigo", "nome", "simples", "datasaida", "cnpj", "ie", "im", "regime", "uf", "mun", "email", "inicio", "situacao", "grupo", "grupo_cod", "cnae_cod", "cnae", "obs"]
...
def buscarempresasativas():
    ...
    _headers = ["codigo", "nome", "simples", "datasaida", "cnpj", "ie", "im", "regime_cod", "uf", "mun", "email", "inicio", "situacao", "grupo", "grupo_cod", "cnae_cod", "cnae", "obs"]
...
def buscarnotaservico(empresa, numero, short=False):
    if short:
        headers = ['codi_emp', 'seri_ser', 'nume_ser', 'dser_ser', 'vcon_ser']
    else:
        headers = getHeaders("efservicos")
    sql = "SELECT nfe.{}\n".format(", nfe.".join(headers))
    sql += "FROM bethadba.efservicos as nfe\n"
    sql += "WHERE nfe.codi_emp = {}\n".format(empresa)
    sql += "  AND nfe.nume_ser = {}\n".format(numero)
    dataset = select(sql, headers)
...
def buscarnotadominio(chave, empresa, short=False):
    ...
    if len(nfe) > 0:
        for segmento in nfe:
            ...
            segmento["aliquotas"] = consultarAliquotasEntrada(segmento["codi_emp"], segmento["codi_ent"])
            segmento["produtos"] = consultarProdutosEntrada(segmento["codi_emp"], segmento["codi_ent"])
            segmento["simples"] = consultarSimplesEntrada(segmento["chave_nfe_ent"], empresa)
    else:
        nfe = consultarSaidaBase(chave, empresa)
        if len(nfe) > 0:
            for segmento in nfe:
                segmento["tipo"] = "saida"
                segmento["aliquotas"] = consultarAliquotasSaida(segmento["codi_emp"], segmento["codi_sai"])
                segmento["produtos"] = consultarProdutosSaida(segmento["codi_emp"], segmento["codi_sai"])
                segmento["simples"] = consultarSimplesSaida(segmento["codi_emp"], segmento["codi_sai"])
...
def buscarnotadominio_deprecated(chave, empresa = ""):
    ...
    consulta = select(sql, ["empresa", "numero", "serie", "valor_produtos", "valor_contabil", "valor_ipi", "situacao", "imposto"])
...
def buscarnotamalha(chave):
    if chave.strip() != "":
        ...
        _sql += "GROUP BY codi_emp, numero, serie, situacao"
        _headers = ["empresa", "numero", "serie", "valor", "situacao", "valor_imposto"]
...
def optsimples(empresa, mes_inicial = 1, ano_inicial = 2017, mes_final = 0, ano_final = 0):
    ...
    while _a < ano_final or (_a == ano_final and _m <= mes_final):
        ...
        if len(_r) > 0:
            _r = _r[0]
            _vigencias[_r["vigencia"].replace("-01", "")] = _r["simples"] == "S"
        else:
...
def consultarvigencias(empresa, mes_inicial = 1, ano_inicial = 0, mes_final = 0, ano_final = 0):
    ...
    if (ano_inicial < 2010):
        tools.Log("Ano inicial inválido:")
        tools.Log("    {}".format(ano_inicial))
        return []
    else:
        _sql = "SELECT vigencia_par, rfed_par FROM bethadba.EFPARAMETRO_VIGENCIA WHERE codi_emp = {} ORDER BY vigencia_par".format(empresa)
        _headers = ["vig", "regime"]
        _datalist = select(_sql, _headers)
        return _datalist
...
def consultarempresasvigenciasbase(empresa = "", mes_inicial = 1, ano_inicial = 0, mes_final = 0, ano_final = 0):
    ...
    _headers = ["codigo", "nome", "simples", "cnpj", "ie", "im", "regime_cod", "uf", "mun", "email", "inicio", "saida", "situacao", "grupo", "grupo_cod", "cnae_cod", "cnae", "fone", "obs"]

    _datalist = select(_sql, _headers)

    for _d in range(len(_datalist)):
        _datalist[_d]["vigencia"] = {}
        _vigencias = consultarvigencias(_datalist[_d]["codigo"], mes_inicial, ano_inicial, mes_final, ano_final)
...
def consultarempresasvigencias(mes, ano):
    return consultarempresasvigenciasbase(empresa = "", mes_inicial = mes, ano_inicial = ano)

def buscarnotasimilar(empresa, serie, numero):
    _sql = "SELECT nfs.codi_emp, nfs.chave_nfe_sai AS chave, nfs.nume_sai AS numero, nfs.seri_sai AS serie, nfs.vprod_sai AS vprod, nfs.situacao_sai AS situacao FROM bethadba.efsaidas nfs WHERE nfs.nume_sai = '{}' AND nfs.seri_sai = '{}' AND nfs.codi_emp = {}".format(numero, serie, empresa)
    _sql = "{}\nUNION\n".format(_sql)
    _sql = "{}SELECT nfe.codi_emp, nfe.chave_nfe_ent AS chave, nfe.nume_ent AS numero, nfe.seri_ent AS serie, nfe.vprod_ent AS vprod, nfe.situacao_ent AS situacao FROM bethadba.efentradas nfe WHERE nfe.nume_ent = '{}' AND nfe.seri_ent = '{}' AND nfe.codi_emp = {}".format(_sql, numero, serie, empresa)
    _headers = ["empresa", "numero", "serie", "valor", "situacao"]
    _datalist = select(_sql, _headers)
    if (len(_datalist) > 0):
        return _datalist[0]
    else:
...
def buscarOptantesSimples(mes_inicial = 0, ano_inicial = 0, mes_final = 0, ano_final = 0):
    _now = datetime.datetime.now()
    if mes_inicial == 0:
        mes_inicial = _now.month
    if ano_inicial == 0:
        ano_inicial = _now.year - 1
    if mes_final == 0:
        mes_final = _now.month
    if ano_final == 0:
        ano_final = _now.year
    ...
    _sql = ""
    _sql += "SELECT emp.codi_emp"

    while _ano < ano_final or (_ano == ano_final and _mes <= mes_final):
        _sql += ",\n"
        _sql += "       '{}-{}-01' = COALESCE((SELECT EFPARAMETRO_VIGENCIA.SIMPLESN_OPTANTE_PAR\n".format(_ano, str(_mes).rjust(2, "0"))
        _sql += "                     FROM bethadba.EFPARAMETRO_VIGENCIA\n"
        _sql += "                    WHERE bethadba.EFPARAMETRO_VIGENCIA.CODI_EMP = emp.codi_emp\n"
        _sql += "                      AND bethadba.EFPARAMETRO_VIGENCIA.VIGENCIA_PAR = (SELECT max(P2.VIGENCIA_PAR )\n"
        _sql += "                                                                          FROM bethadba.EFPARAMETRO_VIGENCIA AS P2\n"
        _sql += "                                                                         WHERE P2.codi_emp = emp.codi_emp\n"
        _sql += "                                                                           AND P2.VIGENCIA_PAR <= '{}-{}-01' )), 'N')".format(_ano, str(_mes).rjust(2, "0"))
        _headers.append("{}/{}".format(str(_mes).rjust(2, "0"), _ano))
    ...
    _mapa = {}
    _data = select(_sql, _headers)
    for _d in _data:
        _mapa[_d["empresa"]] = _d
        del _mapa[_d["empresa"]]["empresa"]
    return _mapa

if __name__ == "__main__":
    @arg_parser(".".join(os.path.basename(__file__).split(".")[0:-1]), 'Executa consultas SQL no banco de dados Sybase da Domínio')
    @positional_param('sql', 'Comando SQL para ser consultado no banco de dados', sample='SELECT * FROM bethadba.usconfusuario', required=True)
    @boolean_param('p', 'Insere o array de resposta em um dicionário para permitir integração com o NodeJS')
    def process():
        dataset = select(params['sql'])
        if params['p']:
            pacote = {
                'dataset': dataset
            }
            print(json.dumps(pacote))
        else:
